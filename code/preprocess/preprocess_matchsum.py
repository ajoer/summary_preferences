...
# -*- coding: utf-8 -*-
"""
Preprocess raw Wikipedia data into MatchSum input format.
"""
...
def save_to_jsonl(output_data, index_data):
	...
	path = "data/summaries/input/matchsum_input/get_candidates"
	with open(f'{path}/en_{gender}_index.jsonl', 'w') as outfile:		
	    for entry in index_data:
	        json.dump(entry, outfile)
	        outfile.write('\n')

	with open(f'{path}/en_{gender}.jsonl', 'w') as outfile:		
...
def main():
	...
	for gender in ["women", "men"]:
		...
		input_data = json.load(open(f"data/summaries/input/wikipedia_raw/en_{gender}_summaries.json"))
